...
class BasicChannel(object):
    ...
    def __init__(self, IPaddress, portNumber, senderID, iface=None):
        ...
        self.seqNo    = 1
        self.createSockets()
        log.info(tr("Connected to group channel {} : {} as {}").format(self.address, self.destPort, self.sender))
    ...
    def createSockets(self):
        ...
        if self.iface is not None:
            self.iface = ipaddress.ip_interface(self.iface)
            log.debug("Multicast only on interface {}".format(self.iface))
            if ipv6:
